...
class CreatePayrollCopyCommandsTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection('db_aux')->create('payroll_copy_commands', function (Blueprint $table) {
            ...
            $table->decimal('sue_formato', 13, 2)->comment('Sueldo formateado');            
            $table->string('cat')->comment('Bono antiguedad');
            $table->decimal('cat_formato', 13, 2)->comment('Bono antiguedad formateado');
            $table->string('est')->comment('Bono estudio');
            $table->decimal('est_formato', 13, 2)->comment('Bono estudio formateado');
            $table->string('carg')->comment('Bono cargo');
            $table->decimal('carg_formato', 13, 2)->comment('Bono cargo formateado');
            $table->string('fro')->comment('Bono frontera');
            $table->decimal('fro_formato', 13, 2)->comment('Bono frontera formateado');
            $table->string('ori')->comment('Bono oriente');
            $table->decimal('ori_formato', 13, 2)->comment('Bono oriente formateado');
            $table->string('bseg')->comment('Bono seguridad');
            $table->decimal('bseg_formato', 13, 2)->comment('Bono seguridad formateado');
            $table->string('gan')->comment('Ganado');
            $table->decimal('gan_formato', 13, 2)->comment('Ganado formateado');
            $table->string('mus')->comment('Total');
            $table->decimal('mus_formato', 13, 2)->comment('Total formateado');
            $table->string('lpag')->comment('Liquido pagado');
            $table->decimal('lpag_formato', 13, 2)->comment('Liquido pagado');
            $table->string('nac')->comment('Fecha de nacimiento');
            $table->date('nac_formato')->comment('Fecha de nacimiento formateado');
            $table->string('ing')->comment('Fecha de ingreso');
            $table->date('ing_formato')->comment('Fecha de ingreso formateado');
            $table->timestamps();
            $table->softDeletes();            
        });
    }
